...
function showMsg(msg) {
    ...
    fromAccountNick = msg.getFromAccountNick(); //用户昵称
    if (!fromAccountNick) {
    ...
    switch (subType) {
        case webim.GROUP_MSG_SUB_TYPE.COMMON://群普通消息
            // contentP.innerHTML = convertMsgtoHtml(msg);
            msgParsed = convertMsgtoHtml(msg);
            break;
        // case webim.GROUP_MSG_SUB_TYPE.REDPACKET://群红包消息
        //     contentP.innerHTML = "[群红包消息]" + convertMsgtoHtml(msg);
        //     break;
        // case webim.GROUP_MSG_SUB_TYPE.LOVEMSG://群点赞消息
        //     //业务自己可以增加逻辑，比如展示点赞动画效果
        //     contentP.innerHTML = "[群点赞消息]" + convertMsgtoHtml(msg);
        //     //展示点赞动画
        //     showLoveMsgAnimation();
        //     break;
        // case webim.GROUP_MSG_SUB_TYPE.TIP://群提示消息
        //     contentP.innerHTML = convertMsgtoHtml(msg);
        //     // contentP.innerHTML = "[群提示消息]" + convertMsgtoHtml(msg);
        //     break;
        default:
            return; //其他消息都忽略
    }
    ...
    switch (msgParsed.subType) {
        case webim.GROUP_MSG_SUB_TYPE.COMMON://群普通消息
            li.setAttribute('class', 'msg-style-chat');
            var div = document.createElement("div");
            div.innerHTML = "<span>" + fromAccountNick + "</span> 说："
            li.appendChild(div);
            break;
        case webim.GROUP_MSG_SUB_TYPE.TIP://群提示消息
            li.setAttribute('class', 'msg-style-sys');
            break;
    }

    contentP.innerHTML = msgParsed.body;
    ...
    for (var i in elems) {
        ...
        switch (type) {
            case webim.MSG_ELEMENT_TYPE.TEXT:
                //解析App端的消息
                try {
                    jstr = JSON.parse(convertTextMsgToHtml(content).replace(/&quot;/g,'"'));
                    var msg = jstr.msg;
                    var userAction = parseInt(jstr.userAction);
                    var nickName = jstr.nickName;
                    var headPic = jstr.headPic;
                    var userId = jstr.userId;

                    if (userAction == 1) { //文本消息
                        msgParsed.subType = webim.GROUP_MSG_SUB_TYPE.COMMON;
                        msgParsed.body = msg;
                    }
                ...
                catch(err) { //纯文本消息（暂且这么认为吧）
                    msgParsed.subType = webim.GROUP_MSG_SUB_TYPE.COMMON;
                    msgParsed.body = convertTextMsgToHtml(content);
                }
